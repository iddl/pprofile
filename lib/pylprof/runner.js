...
class PyLprof {
    ...
    formatTimings(timing) {
        // here we get an array of the following form: [unit, timing, ncalls]
        // unit: unit multiplier for timing, to convert to seconds (eg. 1e-6 for microseconds)
        // timing: total time that was spent on a specific line
        // ncalls: number of times a line was executed
        // we will transform this into
        // [ncalls, avg time spent on line in ms]
        const multiplier = timing[0] / 1e-3;
        return { calls: timing[2], totalTimeMs: timing[1] * multiplier };
    }
        ...
        for (let linenum in data) {
            data[linenum].forEach(l => {
                formatted.push({
                    ...
                    timing: this.formatTimings(l.timing)
                });
            });
        }
